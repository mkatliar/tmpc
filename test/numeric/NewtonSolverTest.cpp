...
namespace tmpc :: testing
{
    ...
    protected:
        ...
        /// @brief Check that solution sensitivities are correct for scalar equation 0=x^2-p
        ///
        template <typename Solver>
        void testSolutionSensitivityScalarQuadratic(Solver& solver)
        {
        ...
        /// @brief Check that solution sensitivities are correct for vector equation 0=A*x-p
        ///
        template <typename Solver>
        void testSolutionSensitivityVectorLinear(Solver& solver)
        {
            size_t constexpr NX = 2;
        ...
        ///
        template <typename Solver>
        void testRosenbrockLineSearch(Solver& solver)
        {
            size_t constexpr NX = 2;
        ...
        template <typename Solver>
        void testRosenbrockMultipleInitialPoints(Solver& solver)
        {
            size_t constexpr NX = 2;
    ...
    TEST_F(NewtonSolverTest, DISABLED_testRosenbrockLineSearchDynamic)
    {
        ...
        testRosenbrockLineSearch(solver);
    }
    ...
    TEST_F(NewtonSolverTest, testRosenbrockMultipleInitialPointsDynamic)
    {
        ...
        testRosenbrockMultipleInitialPoints(solver);
    }


    TEST_F(NewtonSolverTest, testSolveStatic)
    {
        StaticNewtonSolver<Real, 2> solver;
        testSolve(solver);
    }


    TEST_F(NewtonSolverTest, testSolutionSensitivityScalarQuadraticStatic)
    {
        StaticNewtonSolver<Real, 1> solver;
        testSolutionSensitivityScalarQuadratic(solver);
    }


    TEST_F(NewtonSolverTest, testSolutionSensitivityVectorLinearStatic)
    {
        StaticNewtonSolver<Real, 2> solver;
        testSolutionSensitivityVectorLinear(solver);
    }
    ...
    TEST_F(NewtonSolverTest, DISABLED_testRosenbrockLineSearchStatic)
    {
        StaticNewtonSolver<Real, 2> solver;
        solver.maxIterations(20000);
        solver.backtrackingAlpha(0.5);
    ...
    TEST_F(NewtonSolverTest, testRosenbrockMultipleInitialPointsStatic)
    {
        StaticNewtonSolver<Real, 2> solver;
        solver.residualTolerance(1e-10);
        solver.maxIterations(20);
